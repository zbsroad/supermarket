...
public class OutportUpdateFrame extends JFrame {
	 ...
	 * Create the frame.
	 */
	public OutportUpdateFrame(int iid,final OutportQueryFrame father) {
		id = iid;
		setTitle("修改出库信息");
		...
		try {
			outport = outportDao.getById(id);
			Customer c = customerDao.getById(outport.getCid());
			Goods g = goodsDao.getById(outport.getGid());
			codeText.setText(outport.getCode());
			cidBox.setSelectedItem(c);
			gidBox.setSelectedItem(g);
			quantityText.setText(outport.getQuantity()+"");
			priceText.setText(outport.getPrice()+"");
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					if (StringUtil.isEmpty(code)) {
						...
						Outport o = outportDao.getById(id);
					...
					father.fillTable(outportDao.getAll());
				} catch (Exception e1) {
					// TODO Auto-generated catch block
					e1.printStackTrace();
