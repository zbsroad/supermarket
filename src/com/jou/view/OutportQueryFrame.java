...
public class OutportQueryFrame extends JFrame {
	...
	private OutportQueryFrame father = this;
	/**
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					OutportQueryFrame frame = new OutportQueryFrame();
	...
	public OutportQueryFrame() {
		...
		Object[] columns={"ID","出库单","客户","商品","出库单价","出库数量","总价","出库时间"};//字段
		...
		button_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				int id = Integer.valueOf(table.getValueAt(row, 0).toString());
				OutportUpdateFrame update = new OutportUpdateFrame(id,father);
	...
	public void fillTable(List<Outport> list){
		DefaultTableModel tableModel = (DefaultTableModel) table.getModel();
		tableModel.setRowCount(0);// 清除原有行
		// 填充数据
		for(Outport item : list){
			String[] arr=new String[8];
			arr[0]= item.getId()+"";
			arr[1]= item.getCode();
			arr[2]= item.getCustomerName();
