...
public class InportUpdateFrame extends JFrame {
	 ...
	 * Create the frame.
	 */
	public InportUpdateFrame(int iid,final InportQueryFrame father) {
		id = iid;
		setTitle("修改入库信息");
		...
		Inport inport = null;
		try {
			inport = inportDao.getById(id);
			Supplier s = supplierDao.getById(inport.getSid());
			Goods g = goodsDao.getById(inport.getGid());
			codeText.setText(inport.getCode());
			sidBox.setSelectedItem(s);
			gidBox.setSelectedItem(g);
			quantityText.setText(inport.getQuantity()+"");
			priceText.setText(inport.getPrice()+"");
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					father.fillTable(inportDao.getAll());
				} catch (Exception e1) {
					// TODO Auto-generated catch block
					e1.printStackTrace();
